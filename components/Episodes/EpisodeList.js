                                        ...
                                        <List>
                                            {
                                                module.episodes.map((episode, j) => {
                                                    ...
                                                    const bestQuizScoreObj = episode.episode_type === "quiz" && quizBestScores?.find(score => score.quiz_id === episode.id);
                                                    const bestQuizScore = bestQuizScoreObj ? bestQuizScoreObj.best_score : 0;
                                                    const userQuizMessagesToShow = userMessagesClone?.filter(msg => module.Quiz ? msg.quiz_id === module.Quiz.id : null);
                                                    const quizResponses = userQuizMessagesToShow?.filter(msg => (msg.direction === 'from' && typeof msg.message_read === 'undefined'));

                                                    const completed = false; /* student?.completed_episodes?.includes(episode.id); */
                                                    const userMessagesToShow = userMessagesClone && userMessagesClone.filter(msg => msg.quiz_id === null && msg.episode_id === episode.id);
                                                    const responses = userMessagesToShow && userMessagesToShow.filter(msg => msg.direction === 'from' && msg.message_read !== true);
                                                                    ...
                                                                    <List.Item as={'a'}
                                                                        ...
                                                                        <Grid columns={3}>
                                                                            ...
                                                                            <Grid.Column width={10}
                                                                                ...
                                                                                <div
                                                                                    className={props.ctxt === 'course' ? 'truncate-course' : 'truncate-list'}
                                                                                    style={currEpisode.id === episode.id ? {
                                                                                        fontWeight: "700",
                                                                                        color: "#c7c71c"
                                                                                    } : {}}
                                                                                >
                                                                                    {`${episode.quiz?.[`quiz_name`/*`quiz_name_${locale}`*/]}`}
                                                                                </div>
                                                                            </Grid.Column>
                                                                            <Grid.Column width={4}
                                                                                         style={bestQuizScore > 80 ? {color: 'greenyellow'} : {}}>
                                                                                {bestQuizScore === 0 ? '' : `${bestQuizScore}%`}
                                                                            </Grid.Column>
                                                                        </Grid>
                                                                    </List.Item> :
                                                                    <List.Item key={j} as='a'
                                                                               style={{
                                                                                   textAlign: "left",
                                                                                   lineHeight: "1.4em"
                                                                               }}
                                                                            ...
                                                                            >
                                                                                {
                                                                                    ...
                                                                                    <Popup
                                                                                        ...
                                                                                        content={responses.length === 1 ? `1 new message` : `${responses.length} new messages`}
                                                                                        position={'right center'}
                                                                                        inverted/>
                                                                                }
                                                                                <div
